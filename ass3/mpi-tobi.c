    ...
    int my_rank /* in  */)
{
    ...
    do
    {
        ...
        Swap(x_old, x_new); /* altes x wird gegen ein neues x ausgetauscht */
...
*/
    int nFromMatrix = 8;
    int world_size = 8;
    int blocksToRead = nFromMatrix * (nFromMatrix / world_size);
    int numberOfElemMatrix = blocksToRead * nFromMatrix;
    double matrixBuf[numberOfElemMatrix];
    int err = 0, errs = 0;
    ...
    err = MPI_File_open(MPI_COMM_WORLD, "Matrix_A_8x8", MPI_MODE_RDWR | MPI_MODE_CREATE, infoin, &fh);
    if (err)
    {
        errs++;
    ...
    printf("[%d] reading the matrix has finished.\n", my_rank);

    printf("Transforming now to localMatrix-Format.\n");
