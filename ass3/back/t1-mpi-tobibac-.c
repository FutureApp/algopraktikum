...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    err = MPI_File_open(MPI_COMM_WORLD, "Vector_b_8x", MPI_MODE_RDWR | MPI_MODE_CREATE, infoin, &fh);
    if (err)
    {
        errs++;
...
void readMatrix(char *pathToFile, int nOfMatrix, int numberOfElemMatrix)
{
    MPI_Info_create(&infoin);
    MPI_Info_set(infoin, "access_style", "write_once,random");
