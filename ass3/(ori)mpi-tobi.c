...
void Read_vector(char* prompt, float x_local[], int n, int my_rank,
         int p);
void Print_matrix(char* title, MATRIX_T A_local, int n,			////print Matrix Funktion wird nirgendwo benutzt TODO kann gelöscht werden?!
         int my_rank, int p);
...
void Read_matrix(
    ...
    for (i = 0; i < n; i++){ /* temp array wird mit 0.0 aufgefüllt TODO kann später raus */
        for (j = n; j < MAX_DIM; j++){
            temp[i][j] = 0.0;
        }
    }
