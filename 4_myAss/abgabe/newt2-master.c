...
int main(int argc, char *argv[])
{
    ...
    else if (sysInstruc == 1)
    {
        ...
        {
            ptrA = "./test4x4AA.double";
            ptrB = "./test4x4BB.double";
        }

        double localTest[4 * 4];
        int interCounterMY = 1;
        for (i = 17; i <= 4 * 4 *2 ; i++)
        {
        ...
        MPI_File filo;
        char *pathsToResultFile = "./test4x4B.double"; //PATH where to save result
        err = MPI_File_open(MPI_COMM_SELF, pathsToResultFile, MPI_MODE_CREATE | MPI_MODE_RDWR, MPI_INFO_NULL, &filo);
        if (err)
            printf("\nError opening the file.\n");
        MPI_File_write(filo, localTest, 4 * 4, MPI_DOUBLE, MPI_STATUS_IGNORE);
        MPI_File_close(&filo);
        ...
        */
        ...
        // Create childs
        int toSpawnOnDir = sqrt(matrixDim);
        int numberOfChilds = toSpawnOnDir * toSpawnOnDir;
